...
// x = a + b
bool RLWE_Toolkit::add(RingElement& x, RingElement const& a, RingElement const& b)
{
...
// x = a - b
bool RLWE_Toolkit::sub(RingElement& x, RingElement const& a, RingElement const& b)
{
	return neg(x, b) &&	add(x, a, x);
}

// x = -a
bool RLWE_Toolkit::neg(RingElement& x, RingElement const& a)
{
	x = coordinate_type(-1) * a;
...
// x = a * b
bool RLWE_Toolkit::mult(RingElement& x, RingElement const& a, RingElement const& b)
{
	// possible cases: modulus = max(0,0), modulus = max(q,0), 
	// modulus = max(0,q), modulus = max(q,q)
	pos_int modulus = std::max(a.getPrimeModulusQ(), b.getPrimeModulusQ());
...
// x = scalar * b
bool RLWE_Toolkit::multWithScalar(RingElement& x, const coordinate_type scalar, RingElement const& a)
{
...
{
	RingElement x(a.getFieldSptr());
	RLWE_Toolkit::mult(x, a, b);
	return x;
}

RingElement operator*(const coordinate_type scalar, RingElement const& b)
{
	RingElement x(b.getFieldSptr());
	RLWE_Toolkit::multWithScalar(x, scalar, b);
	return x;
}
...
// x = t * a
bool RLWE_Toolkit::multWithT(RingElement& x, RingElement const& a)
{
	if (x != a){
		x = a;
	}
...
// x = t^-1 * a
bool RLWE_Toolkit::multWithT_Inverse(RingElement& x, RingElement const& a)
{
	Basis b = a.getBasis();
	x = a;
...
void RLWE_Toolkit::mod(RingElement& x, RingElement const& a, int q)
{
...
void RLWE_Toolkit::mod(coordinate_vec& x, coordinate_vec const& a, int q)
{
	if (x.size() != a.size()){
		x.resize(a.size());
	}
...
void RLWE_Toolkit::decode(RingElement& x, RingElement const& a)
{
	if (x != a){
		x = a;
	}
...
bool RLWE_Toolkit::computeUniqueRepresentative(RingElement& x, RingElement const& a)
{
...
void RLWE_Toolkit::discretize(RingElement& x, real_vec& realCoordinateVector, RingElement const& cosetRepresentative, int const scalingFactor)
{
...
void RLWE_Toolkit::sampleDiscretizedGaussian(RingElement& x, real_type mean, real_type stddev, RingElement const& cosetRepresentative,
...
void RLWE_Toolkit::sampleGaussiansInK_RR(real_vec& x, LweField const& field, real_type mean, real_type stddev)
{
...
coordinate_type RLWE_Toolkit::sampleDiscreteGaussianInZZ(real_type stddev, real_type center, real_type funcValue /*= 3*/)
{
	using namespace std;
...
void RLWE_Toolkit::sampleDiscreteGaussianInR(RingElement& x, RingElement const& c, real_type stddev)
{
	LweField const& field = x.getField();
	int n = field.getDimension();
...
void RLWE_Toolkit::sampleUniformlyCoordVec(RingElement& x, coordinate_type lb, coordinate_type ub)
{
	using namespace std;
...
void RLWE_Toolkit::convertIntVecToCompVec(comp_vec& compVec, coordinate_vec const& intVec)
{
...
void RLWE_Toolkit::convertCompVecToIntVec(coordinate_vec& intVec, comp_vec const& compVec)
{
...
std::ostream& operator<<(std::ostream& out, RLWE_Toolkit::Main_Classes::RingLweCryptographyElement const& elmt)
{
	using namespace std;
	string basis;
